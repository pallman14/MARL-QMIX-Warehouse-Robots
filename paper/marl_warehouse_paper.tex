...
QMIX achieved significantly higher returns than independent learning approaches. On tiny-2ag-v2, QMIX (3.25) outperformed advanced IPPO (0.38) by approximately 755\%, representing an 8.5x improvement in task completion. Even vanilla IPPO with 4x longer training (20M vs 5M steps) achieved only 0.13 mean return---25x lower than QMIX. These results demonstrate that value decomposition provides substantial advantages for sparse-reward coordination where credit assignment is challenging. Note: MASAC was evaluated on MPE (achieving 63\% improvement over random baseline) but not on RWARE (\href{https://pallman14.github.io/MARL-QMIX-Warehouse-Robots/lian/week1.html}{MPE results}). Detailed learning curves are available in our \href{https://pallman14.github.io/MARL-QMIX-Warehouse-Robots/lian/week4.html}{training analysis}.
...
For researchers attempting QMIX on RWARE or similar sparse-reward warehouse tasks, we distill our key lessons. \textbf{Most critical}: extend epsilon annealing to at least 5M steps (100x default)---premature greedy behavior prevents agents from ever discovering sparse rewards. \textbf{Second}: increase replay buffer size substantially (we used 200K vs 5K default) to maintain experience diversity across long training horizons. \textbf{Third}: plan for 10--20M training steps minimum; 2M steps consistently produced zero learning regardless of other hyperparameters.

\textbf{Recommended starting configuration for RWARE+QMIX}: batch size 256, buffer size 200K, epsilon annealing 5M steps, learning rate 0.0005, GRU hidden dimension 64, total training 20M steps. Our experiments ran on Mac M3 (16GB RAM) for RWARE and Windows Server 2022 (Intel Xeon E5-2680, 196GB RAM) for Unity; the latter completed 1M Unity steps in approximately 6 hours using no-graphics mode with 50x time scaling.
