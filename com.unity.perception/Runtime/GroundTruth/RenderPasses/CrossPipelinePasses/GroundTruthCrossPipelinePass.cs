...
namespace UnityEngine.Perception.GroundTruth
{
    abstract class GroundTruthCrossPipelinePass : IGroundTruthGenerator
    {
        ...
        protected static RendererList CreateRendererListFromDesc(
            ScriptableRenderContext renderContext, RendererListDesc desc)
        {
#if CORE_RP_17_OR_NEWER
            return renderContext.CreateRendererList(desc);
#else
            return RendererList.Create(desc);
#endif
        }
...
#if CORE_RP_17_OR_NEWER
            // Unity 6+ uses CommandBuffer to draw RendererList
            cmd.DrawRendererList(rendererList);
            renderContext.ExecuteCommandBuffer(cmd);
            cmd.Clear();
#else
            // Legacy API: manually extract and draw using DrawRenderers
